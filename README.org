...
gptel can be installed in Emacs out of the box with =M-x package-install= ⏎ =gptel= or with =use-package= in your =init.el=.  With the default Emacs configuration, this will install the latest released (stable) version from NonGNU ELPA.  Released versions are available from MELPA Stable as well.

If you want the latest development snapshot instead, add MELPA to your list of package sources (=package-archives=).
